...
pub struct DualGrid {
    ...
    /// They are always presented in the same order: The same one as defined in [`crate::util::OctreeIndex`].
    ///
    /// ## Notes
    /// Some cells will have shared vertices, however, their topology will be the same one as a cube, so the
    /// Marching Cubes algorithm will work over them.
...
impl DualGrid {
    /// Constructs a dual grid from an octree.
